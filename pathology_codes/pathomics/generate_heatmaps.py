...
import pandas as pd
from pathomics.support.utils import *
from math import floor
from pathomics.support.eval_utils import initiate_model as initiate_model
from pathomics.models.model_clam import CLAM_MB, CLAM_SB
from pathomics.models.resnet_custom import resnet50_baseline
from types import SimpleNamespace
from collections import namedtuple
import h5py
import yaml
from pathomics.slides.batch_process_utils import initialize_df
from pathomics.visualization.heatmap_utils import initialize_wsi, drawHeatmap, compute_from_patches
from pathomics.slides.wsi_utils import sample_rois
...
parser.add_argument('--config-file', type=Path, required=True, help='YAML configuration for this pathology heatmap run')
...
if __name__ == '__main__':
	if not cli_args.config_file.is_file():
		raise FileNotFoundError(f'Heatmap configuration not found: {cli_args.config_file}')
	with cli_args.config_file.open('r', encoding='utf-8') as config_stream:
		config_dict = yaml.safe_load(config_stream)
	config_dict = parse_config_dict(cli_args, config_dict)
	...
	if data_args.process_list is None:
		...
		if not os.path.isabs(process_list_path):
			# 兼容原代码的相对路径逻辑，或者您可以直接删掉这行，只支持绝对路径
		...
		if not os.path.exists(process_list_path):
			raise FileNotFoundError(f"Cannot find process list file: {process_list_path}")
	...
	for i in range(len(process_stack)):
		...
		process_report_directory = os.path.join(exp_args.raw_save_dir, exp_args.save_exp_code)
		os.makedirs(process_report_directory, exist_ok=True)
		if data_args.process_list is not None:
    			# 修改开始：提取纯文件名
			list_name = os.path.splitext(os.path.basename(data_args.process_list))[0]
			process_stack.to_csv(os.path.join(process_report_directory, '{}.csv'.format(list_name)), index=False)
   			 # 修改结束
		else:
			process_stack.to_csv(os.path.join(process_report_directory, '{}.csv'.format(exp_args.save_exp_code)), index=False)
		...
		if heatmap_args.save_orig:
			...
			save_dir_for_slide = os.path.join(p_slide_save_dir, slide_id)
			os.makedirs(save_dir_for_slide, exist_ok=True)
			full_save_path = os.path.join(save_dir_for_slide, heatmap_save_name)
			if os.path.isfile(full_save_path):
				...
				if heatmap_args.save_ext == 'jpg':
					heatmap.save(full_save_path, quality=100)
				else:
					heatmap.save(full_save_path)
